...
WS_BLACKLIST_TTL = 300.0
WS_FAIL_TIMEOUT = 2.0
LISTENER_CHECK_INTERVAL = 5.0
LISTENER_RESTART_DELAY = 1.0
GRACEFUL_SHUTDOWN_TIMEOUT = 5.0
MAX_CLIENTS = 1024
CLIENT_READ_TIMEOUT = 300.0

ws_blacklist: Dict[str, float] = {}
dc_fail_until: Dict[str, float] = {}
ip_fail_until: Dict[str, float] = {}
_client_semaphore: Optional[asyncio.Semaphore] = None
...
def _cleanup_blacklists(now: float) -> None:
    expired = [k for k, t in ws_blacklist.items() if now >= t]
    for k in expired:
        del ws_blacklist[k]
...
async def _handle_client(reader, writer, secret: bytes):
    global _client_semaphore
    if _client_semaphore is not None:
        await _client_semaphore.acquire()
    try:
        await _handle_client_inner(reader, writer, secret)
    finally:
        if _client_semaphore is not None:
            _client_semaphore.release()


async def _handle_client_inner(reader, writer, secret: bytes):
    stats.incr('connections_total')
    stats.incr('connections_active')
    ...
    try:
        ...
        if ws is None:
            ...
            if ws_failed_redirect and all_redirects:
                ...
                log.warning("[%s] DC%d%s blacklisted for WS (all 302) for %ds",
                            label, dc, media_tag, int(WS_BLACKLIST_TTL))
        ...
        stats.incr('connections_active', -1)
        try:
            writer.close()
...
async def _run(stop_event: Optional[asyncio.Event] = None):
    global _server_instance, _server_stop_event, _client_semaphore
    _server_stop_event = stop_event
    _client_semaphore = asyncio.Semaphore(MAX_CLIENTS)
    ...
    try:
        while True:
            ...
            if stop_task is not None and stop_task in done:
                log.info("Shutdown requested, draining %d connections...",
                         len(_client_tasks))
                # Graceful drain: let in-flight tasks finish with timeout
                if _client_tasks:
                    _, pending = await asyncio.wait(
                        _client_tasks, timeout=GRACEFUL_SHUTDOWN_TIMEOUT)
                    if pending:
                        log.warning("Force-cancelling %d connections after %.0fs timeout",
                                    len(pending), GRACEFUL_SHUTDOWN_TIMEOUT)
                        for t in pending:
                            t.cancel()
