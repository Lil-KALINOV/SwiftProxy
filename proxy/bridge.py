...
class MsgSplitter:
    ...
    def split(self, chunk: bytes) -> List[bytes]:
        ...
        # Guard against unbounded buffering: if we can't frame a packet
        # and the buffered data keeps growing, disable splitting and pass
        # the whole stream through as raw frames.
        if len(self._cipher_buf) + len(chunk) > self.MAX_BUF:
            self._disabled = True
            tail = bytes(self._cipher_buf) + chunk
...
async def _cfproxy_worker_fallback(reader, writer, relay_init, label,
    ...
    stats.incr('connections_cfproxy')
...
async def _cfproxy_fallback(reader, writer, relay_init, label,
    ...
    for base_domain in balancer.get_domains_for_dc(dc):
        ...
        try:
            ...
            balancer.report_domain_failure(base_domain)
...
async def _tcp_fallback(reader, writer, dst, port, relay_init, label, ctx: CryptoCtx):
    ...
    stats.incr('connections_tcp_fallback')
    rw.write(relay_init)
    await rw.drain()
        ...
        try:
            while True:
                try:
                    ...
                    close_reason = 'upstream: read_timeout'
                    break
                if data is None:
