...
class _WsPool:
    ...
    @staticmethod
    def _is_stale(ws) -> bool:
        try:
            if ws._closed or ws.writer.transport.is_closing():
                return True
        except Exception:
            return True
        sock = None
    ...
    async def get(self, dc: int, is_media: bool,
        ...
        while bucket:
            ...
            age = now - created
            if age > self.WS_POOL_MAX_AGE or self._is_stale(ws):
                asyncio.create_task(self._quiet_close(ws))
                continue
            stats.incr('pool_hits')
    ...
    async def _refill(self, key, target_ip, domains):
        ...
        try:
            ...
            total_idle = sum(len(q) for q in self._idle.values())
            if total_idle >= self.IDLE_MAX_TOTAL:
                return
            needed = min(
                proxy_config.pool_size - len(bucket),
                self.IDLE_MAX_TOTAL - total_idle,
            )
            if needed <= 0:
                return
            connected = 0
    ...
    async def _connect_fronted(self, target_ip, domain) -> Optional[RawWebSocket]:
        ...
        stats.incr('connections_fronting')
        self.try_fronting_first = True
        return ws
