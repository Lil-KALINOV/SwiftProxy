...
def _legacy_app_dir() -> Optional[Path]:
    if sys.platform == "win32":
        return Path(os.environ.get("APPDATA", Path.home())) / LEGACY_APP_NAME
...
def _migrate_from_legacy() -> None:
    """Copy config.json from the old (pre-rename) data dir if present."""
    ...
    if not legacy_cfg.exists():
        return
    try:
        data = legacy_cfg.read_bytes()
        APP_DIR.mkdir(parents=True, exist_ok=True)
        CONFIG_FILE.write_bytes(data)
    except OSError as exc:
        log.warning("Legacy config migration failed: %s", repr(exc))
