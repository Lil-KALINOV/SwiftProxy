...
def _segmented_if_possible(ctk, parent, theme, values, variable, command):
    """iOS segmented picker; falls back to nothing if API unavailable."""
    try:
...
def _checkbox(ctk, parent, theme, text, variable, tip=""):
    """iOS-style toggle: label on the left, white-thumb switch on the right."""
    ...
    switch = ctk.CTkSwitch(
        ...
        corner_radius=13,
        border_width=0,
        fg_color=theme.switch_on,
        progress_color=theme.switch_off,
        button_color=theme.switch_thumb,
        button_hover_color=theme.switch_thumb,
    )
    lbl = ctk.CTkLabel(
        row,
        text=text,
...
def _labeled_entry(ctk, parent, theme, label_text, value, *, tip="", width=0, pack_fill=False):
    ...
    lbl = _label(ctk, col, theme, label_text, size=13, secondary=False)
...
def _config_section(
    ...
    *,
    ...
    ctk.CTkFrame(title_row, fg_color=theme.accent_soft,
                 width=4, height=14, corner_radius=2).pack(side="left", padx=(0, 7))
    _label(ctk, title_row, theme, title, secondary=False, bold=True, size=13).pack(side="left")
    card = ctk.CTkFrame(
        wrap, fg_color=theme.field_bg, corner_radius=theme.corner_radius,
    ...
    *,
    ...
    def _logo_image():
        try:
            ...
            return render_icon(92)
        except Exception:
            return None

    if (logo := _logo_image()) is not None:
        ctk.CTkLabel(
            header, text="", image=ctk.CTkImage(logo, size=(46, 46)),
        ).pack(side="left", padx=(0, 10))
    else:
        ctk.CTkFrame(
            header, fg_color=theme.accent, width=46, height=46,
            corner_radius=12,
        ).pack(side="left", padx=(0, 10))
    ...
    ctk.CTkLabel(
        title_col, text="SwiftProxy",
        font=(theme.ui_font_family, 18, "bold"),
        text_color=theme.text_primary, anchor="w",
    ).pack(anchor="w")
    ver_badge = ctk.CTkFrame(
        title_col, fg_color=theme.accent, corner_radius=7,
    )
    ver_badge.pack(anchor="w", pady=(2, 0))
    ctk.CTkLabel(
        ver_badge, text=f" v{__version__} ",
        font=(theme.ui_font_family, 11, "bold"),
    ...
    theme_seg = ctk.CTkSegmentedButton(
        ...
        unselected_color=theme.bg,
        unselected_hover_color=theme.field_border,
        text_color=theme.text_primary,
        command=_on_appearance_change,
    )
    theme_seg.pack(fill="x")
    ...
    dc_textbox = ctk.CTkTextbox(
        ...
        fg_color=theme.field_inset, border_color=theme.field_border,
        border_width=1, text_color=theme.text_primary,
    )
    ...
    *,
    ...
    if logo is not None:
        ctk.CTkLabel(
            title_frame,
            text="",
            image=ctk.CTkImage(logo, size=(44, 44)),
        ).pack(side="left", padx=(0, 10))
    else:
        ctk.CTkFrame(title_frame, fg_color=theme.accent,
                     width=44, height=44, corner_radius=12).pack(side="left", padx=(0, 10))

    title_col = ctk.CTkFrame(title_frame, fg_color="transparent")
    title_col.pack(side="left", fill="x", expand=True)
    ctk.CTkLabel(title_col, text="SwiftProxy",
                 font=(theme.ui_font_family, 17, "bold"),
                 text_color=theme.text_primary).pack(anchor="w")
    ctk.CTkLabel(title_col, text=t("first_run.title"),
                 font=(theme.ui_font_family, 12),
    ...
    _checkbox(ctk, frame, theme, t("first_run.open_now"),
              auto_var).pack(anchor="w", fill="x", pady=(0, 18))

    def on_ok():
        on_done(auto_var.get())

    ctk.CTkButton(frame, text=t("button.start"), width=180, height=42,
                  font=(theme.ui_font_family, 15, "bold"), corner_radius=14,
                  fg_color=theme.accent, hover_color=theme.accent_hover,
                  text_color="#ffffff",
